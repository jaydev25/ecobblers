...
function parse(rawArgv) {
  ...
  var parser = (_yargsInstance$config = yargsInstance.config(getCLIConfig()).usage('Usage: $0 [options] <script>...').example('$0 test build', 'Runs the `test` script then the `build` script').example('$0 "test --cover" "build --prod"', _commonTags.oneLine`
        Runs the \`test\` script and forwards the "--cover" flag
        then the \`build\` script and forwards the "--prod" flag
      `).help(false).alias('h', 'help').version().alias('v', 'version').options(baseOptions)).command.apply(_yargsInstance$config, _toConsumableArray(getInitCommand())).completion('completion', completionHandler).exitProcess(shouldExitProcess());
...
function getCLIConfig() {
  var configPath = _findUp2.default.sync('.npsrc') || _findUp2.default.sync('.npsrc.json');

  if (!configPath) {
    return {};
  }

  return (0, _binUtils.loadCLIConfig)(configPath);
}
